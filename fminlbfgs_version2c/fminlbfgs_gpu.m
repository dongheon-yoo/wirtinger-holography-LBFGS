function [x,fval,exitflag,output,grad]=fminlbfgs_gpu(funfcn,x_init,optim)
...
%	- Quasi Newton Broyden뺽letcher뺾oldfarb뻊hanno (BFGS)  
...
%	    OPTIONS.HessUpdate : If set to 'bfgs', Broyden뺽letcher뺾oldfarb뻊hanno 
...
if(optim.HessUpdate(1)=='b'), output.algorithm='Broyden뺽letcher뺾oldfarb뻊hanno (BFGS)';
...
if ((deltaX'*deltaG) >= sqrt(eps)*max( eps,norm(deltaX)*norm(deltaG) ))
    ...
    if(optim.HessUpdate(1)=='b')
        ...
        data.deltaX(:,2:optim.StoreN)=data.deltaX(:,1:optim.StoreN-1); data.deltaX(:,1)=extractGPU(deltaX);
        data.deltaG(:,2:optim.StoreN)=data.deltaG(:,1:optim.StoreN-1); data.deltaG(:,1)=extractGPU(deltaG);
    
        data.nStored=data.nStored+1; if(data.nStored>optim.StoreN), data.nStored=optim.StoreN; end

        % Initialize variables
        a=convertGPU(zeros(1,data.nStored));
        p=convertGPU(zeros(1,data.nStored));
